...
class route(object):
  def __init__(self, route_id, name, length, elevation_gain, route_type, sub_type, popularity, starting_location, ending_location, athlete_id):
    ...
    self.end_latitude = ending_location.latitude
    self.end_longitude = ending_location.longitude
    self.ending_point_geo_asset = {'name': "", 'cc': "", 'admin1': "", 'admin2': ""}
    self.start_end_separation_in_meters = self.compute_start_end_separation_in_meters(starting_location, ending_location)
    self.athlete_id = athlete_id

  def compute_start_end_separation_in_meters(self, starting_location, ending_location):
    return vincenty((starting_location.latitude, starting_location.longitude), (ending_location.latitude, ending_location.longitude))*1000
